///Register `GPTIMER1CTRL` reader
pub type R = crate::R<Gptimer1ctrlSpec>;
///Register `GPTIMER1CTRL` writer
...
///Field `GPTRST` writer - GPTRST General Purpose Timer Reset 0 - No action 1 - Load counter value from GPTLD bits in USB_n_GPTIMER1LD
...
impl W {
    ///Bit 24 - GPTMODE General Purpose Timer Mode In one shot mode, the timer will count down to zero, generate an interrupt, and stop until the counter is reset by software. In repeat mode, the timer will count down to zero, generate an interrupt and automatically reload the counter value from GPTLD bits to start again. 0 - One Shot Mode 1 - Repeat Mode
    #[inline(always)]
    pub fn gptmode(&mut self) -> GptmodeW<'_, Gptimer1ctrlSpec> {
        GptmodeW::new(self, 24)
    }
    ///Bit 30 - GPTRST General Purpose Timer Reset 0 - No action 1 - Load counter value from GPTLD bits in USB_n_GPTIMER1LD
    #[inline(always)]
    pub fn gptrst(&mut self) -> GptrstW<'_, Gptimer1ctrlSpec> {
        GptrstW::new(self, 30)
    }
    ///Bit 31 - GPTRUN General Purpose Timer Run GPTCNT bits are not effected when setting or clearing this bit. 0 - Stop counting 1 - Run
    #[inline(always)]
    pub fn gptrun(&mut self) -> GptrunW<'_, Gptimer1ctrlSpec> {
        GptrunW::new(self, 31)
    }
}
...
///`read()` method returns [`gptimer1ctrl::R`](R) reader structure
impl crate::Readable for Gptimer1ctrlSpec {}
///`write(|w| ..)` method takes [`gptimer1ctrl::W`](W) writer structure
impl crate::Writable for Gptimer1ctrlSpec {
    type Safety = crate::Unsafe;
}
///`reset()` method sets GPTIMER1CTRL to value 0
